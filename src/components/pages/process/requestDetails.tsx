...
function RequestDetails(props: any) {
    ...
    const requestDatas = [{ name: "Off Node", Sample_Quality: "test1x", Incharge_Name: "ytestxdf", Office_Sample_Cake_ID: "hbsckjdvh" }, { name: "Store Node 2", test1g: "test1x", ytestgh: "ytestxddf" }, { name: "Node X" }]
    return (
        <div className="mx-[5%] my-10">
            ...
            <div className="flex justify-between items-center my-6">
                <div className="flex items-center gap-x-1">
                    <HeaderText text={`Sample Request Tracking /`} onClick={()=>{navigate(-1)}} classes={`cursor-pointer`} />
            ...
            <div className="detailsContainer flex flex-col gap-y-4 my-4">
                {requestDatas.map((item: any, index: number) => {
                    return <div className="my-2"><RequestCard key={index} data={item} index={index} /></div>
                })}
            </div>
        </div>
    )
